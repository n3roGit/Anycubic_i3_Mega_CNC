    ...
    case 'G': switch (parser.codenum) {
      ...
      case 28: G28(); break;                                      // G28: Home one or more axes
        ...
        case 29:                                                  // G29: Bed leveling calibration
          ...
          break;
        //case 999: // Beispiel: G999 zum Einstellen der PWM für den Motor
        //G999(); break;
      #endif
...
/*void GcodeSuite::G999() {
  if (parser.seen('S')) {
    int pwm_value = parser.value_int();
    pwm_value = constrain(pwm_value, 0, 255); // Beschränken Sie den Wert auf gültige PWM-Bereiche
    analogWrite(SPINDLE_CONTROL_PIN, pwm_value); // Setzen Sie den PWM-Wert
    SERIAL_ECHO_START();
    SERIAL_ECHOLNPAIR("Set Motor PWM to ", pwm_value);
  } else {
    // Fehlermeldung, wenn kein S-Parameter angegeben wurde
    SERIAL_ERROR_START();
    SERIAL_ERRORLNPGM("S Parameter Missing");
  }
}*/
